...
class TTS:
    ...
    def run(self, inputs:dict):
        ...
        if (ref_audio_path is not None) and (ref_audio_path != self.prompt_cache["ref_audio_path"]):
            # 获取并打印绝对路径
            absolute_ref_audio_path = os.path.abspath(ref_audio_path)
            print(f"Reference audio path: {absolute_ref_audio_path}")

            if not os.path.exists(absolute_ref_audio_path):
                raise ValueError(f"{ref_audio_path} does not exist")
